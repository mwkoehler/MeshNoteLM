...
public class GrokPlugin : AIProviderPluginBase
{
    ...
    private async Task<string> GetAvailableModelsFromApiAsync()
    {
        try
        {
            // Try to get models from xAI API (OpenAI-compatible)
            ...
            // Fallback to testing common Grok models
            return await TestCommonGrokModelsAsync();
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[GrokPlugin] Error getting models from API: {ex.Message}");
    ...
    /// <summary>
    /// Test common Grok models as fallback
    /// </summary>
    private async Task<string> TestCommonGrokModelsAsync()
    {
        try
        {
            ...
            foreach (var model in commonModels)
            {
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"[GrokPlugin] Found working model: {model}");
                    }
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"[GrokPlugin] Model {model} failed: {ex.Message}");
                }
            }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[GrokPlugin] Error testing common models: {ex.Message}");
